...
export function PixelSlider({
  ...
  // Compute suggested marker position as percentage
  const suggestedPct =
  ...
  return (
    <div className="flex flex-col gap-1.5">
      ...
      {/* Slider track wrapper — relative so we can overlay suggested marker */}
      <div className="relative">
        ...
        {/* Suggested value marker — gold tick above the track */}
        {suggestedPct !== undefined && (
          ...
          >
            <span className="font-pixel text-[8px] text-[#ffd700] glow-gold whitespace-nowrap">
              ★
            </span>
          </div>
        )}
      ...
      {/* Hint text */}
      {hint && (
        <p className="font-pixel text-[9px] text-[#708090] leading-relaxed">
          {hint}
        </p>
      )}
    </div>
